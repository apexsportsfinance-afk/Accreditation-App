...
export const EventCategoriesAPI = {
  getByEventId: async (eventId) => {
    const data = await handleResponse(
      supabase.from("event_categories").select("*, categories(*)").eq("event_id", eventId)
    );
    return (data || []).map(r => ({
      id: r.id,
      eventId: r.event_id,
...
// ============================================================
// ⬇️ ADD YOUR EDGE FUNCTION HELPER HERE — NO CHANGES NEEDED
// ============================================================
const callUserFunction = async (action, payload) => {
  const { data: { session } } = await supabase.auth.getSession();
  if (!session) throw new Error("Not authenticated");

  const response = await fetch(
    // ✅ THIS USES YOUR VITE_SUPABASE_URL ENV VARIABLE AUTOMATICALLY
    `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/manage-users`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        // ✅ THIS USES YOUR VITE_SUPABASE_ANON_KEY ENV VARIABLE AUTOMATICALLY
...
// ============================================================
// ⬆️ END OF HELPER — NO API KEYS NEEDED, USES ENV VARIABLES
// ============================================================
...
export const UsersAPI = {
  ...
  create: async (userData) => {
    const result = await callUserFunction("create", {
      name: userData.name,
  ...
  // ✅ NOW USES EDGE FUNCTION
  update: async (id, updates) => {
    await callUserFunction("update", { id, ...updates });
