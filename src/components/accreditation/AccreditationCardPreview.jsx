...
import { getCountryName, calculateAge, COUNTRIES, isExpired, formatDate } from "../../lib/utils";
...
const getLastNameSize = (lastName) => {
  const len = (lastName || "").length;
  if (len > 18) return "text-[18px] leading-[1.1]";
  if (len > 14) return "text-[22px] leading-[1.1]";
  if (len > 10) return "text-[26px] leading-[1.1]";
  return "text-[30px] leading-[1.1]";
};

// Generate a zone badge as a raster PNG so html2canvas renders it pixel-perfect
const useZoneBadgePngs = (codes) => {
  const [badges, setBadges] = React.useState({});
  React.useEffect(() => {
    if (!codes || codes.length === 0) return;
    const result = {};
    const size = 44; // 22px * 2x scale
    codes.forEach((code) => {
...
const AccreditationCardPreview = ({ accreditation, event, zones = [] }) => {
  ...
  const roleData = zoneColor
    ? { bg: "", hex: zoneColor }
    : getRoleData(accreditation?.role);
  ...
  const lastNameSize = getLastNameSize(accreditation?.lastName);
  ...
  return (
    <div id="accreditation-card-preview" className="flex flex-col lg:flex-row gap-6 items-start justify-center font-sans">
      {/* FRONT CARD - Exact layout for PDF capture */}
      <div id="accreditation-front-card" style={{ width: "320px", height: "454px", backgroundColor: "#ffffff", borderRadius: "0", boxShadow: "0 25px 50px -12px rgba(0, 0, 0, 0.25)", overflow: "hidden", flexShrink: 0, display: "flex", flexDirection: "column", position: "relative", border: expired ? "2px solid #f87171" : "1px solid #e2e8f0" }}>
        {/* Expired Overlay */}
        {expired && (
        ...
        {/* Small gap between header and role banner */}
        ...
        <div
          style={{
            ...
            flexShrink: 0,
            background: zoneColor ? zoneColor : "linear-gradient(to right, #7c3aed, #8b5cf6, #a855f7)"
          }}
        >
        ...
        <div style={{ display: "flex", flex: 1, padding: "12px", position: "relative", zIndex: 10, minHeight: 0, backgroundColor: "white" }}>
          ...
          {/* RIGHT: Name, Club, Role, details - EXPANDED TO FILL SPACE */}
          <div style={{ flex: 1, paddingLeft: "16px", display: "flex", flexDirection: "column", justifyContent: "flex-start", minWidth: 0 }}>
            {/* First Name */}
            <p style={{ fontSize: "15px", color: "#1e293b", fontWeight: 500, lineHeight: 1.2 }}>
              {accreditation?.firstName || "First Name"}
            </p>
            {/* Last Name - LARGER */}
            <h2 style={{ fontWeight: "bold", color: "#0f172a", textTransform: "uppercase", marginTop: "2px", fontSize: lastNameSize.includes("30") ? "30px" : lastNameSize.includes("26") ? "26px" : lastNameSize.includes("22") ? "22px" : "18px", lineHeight: 1.1 }}>
              {accreditation?.lastName || "LAST NAME"}
            </h2>

            {/* Club - with more spacing */}
            ...
            {/* Country with Flag - LARGER flag and more prominent */}
        ...
        <div style={{ height: "36px", width: "100%", borderTop: "1px solid #e2e8f0", display: "flex", alignItems: "center", justifyContent: "center", gap: "12px", padding: "0 12px", flexShrink: 0, backgroundColor: "#f8fafc" }}>
          ...
          {event?.sponsorLogos?.length > 0 ? (
            event.sponsorLogos.slice(0, 6).map((logo, index) => (
              logo ? <img key={index} src={logo} alt="Sponsor" style={{ height: "26px", maxWidth: "50px", objectFit: "contain" }} /> : null
            ))
          ) : (
      ...
      {/* BACK CARD */}
      <div id="accreditation-back-card" style={{ width: "320px", height: "454px", background: "linear-gradient(to bottom right, #0f172a, #1e293b, #0f172a)", borderRadius: "0", boxShadow: "0 25px 50px -12px rgba(0, 0, 0, 0.25)", overflow: "hidden", flexShrink: 0, border: "1px solid #334155", position: "relative" }}>
