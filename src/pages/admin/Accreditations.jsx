...
export default function Accreditations() {
  ...
  const handleDownloadImages = useCallback(async (accreditation) => {
    ...
    try {
      ...
      await downloadBothCardsAsImages("accreditation-front-card", "accreditation-back-card", baseFileName, selectedImageSize);
  ...
  const handleDelete = async (id) => {
    await AccreditationsAPI.delete(id);
  ...
  const confirmApprove = async () => {
    ...
    try {
      ...
      // Query DB for exact count of approved accreditations for this role+event
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          ...
          <p className="text-lg text-slate-400 font-extralight">
            Review and manage participant accreditations
          </p>
        </div>
        <div className="flex items-center gap-3">
          <Button
            ...
            icon={RefreshCw}
            onClick={async () => {
              const data = await AccreditationsAPI.getByEventId(selectedEvent);
      ...
      >
        {viewModal.accreditation && (
          <div className="p-6 space-y-6">
            ...
            {viewModal.accreditation.status === "pending" && (
              <div className="flex gap-3">
                <Button
                  variant="success"
                  icon={CheckCircle}
